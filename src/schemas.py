...
class UserBase(BaseModel):
    username: str = Field(
        ..., min_length=3, max_length=50, json_schema_extra={"example": "john_doe"}
    )
...
class TrackBase(BaseModel):
    ...
    description: str = Field(
        ...
        json_schema_extra={
            "example": "Энергичный ностальгический синтипоп 80-х с яркими синтезаторами, быстрым ритмом и меланхоличным вокалом."
        },
    )
...
class PlaylistCreate(BaseModel):
    ...
    track_ids: list[int] = Field(
        ..., description="Список ID треков для включения в плейлист"
    )
...
class SemanticSearchRequest(BaseModel):
    query: str = Field(
        ...
        json_schema_extra={
            "example": "Грустная акустическая гитара для дождливого вечера"
        },
    )
